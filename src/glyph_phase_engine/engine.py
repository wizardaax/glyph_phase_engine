...
class GlyphPhaseEngine:
    ...
    applications. The basic implementation uses simple heuristics
    (input length, delta magnitude), but subclasses can implement
    ...
    def adjust_phase_delta(self, delta_value: float) -> PhaseState:
        ...
        Args:
            delta_value: The delta adjustment value (can be positive
                or negative)
        ...
        if self.current_phase == PhaseState.DELTA_ADJUSTMENT:
            ...
            if abs(delta_value) < _SMALL_DELTA_THRESHOLD:
                # Small delta indicates convergence
                self.current_phase = PhaseState.STABILIZED
            elif abs(delta_value) > _LARGE_DELTA_THRESHOLD:
